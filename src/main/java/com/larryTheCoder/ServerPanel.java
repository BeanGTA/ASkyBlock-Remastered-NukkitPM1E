...
public class ServerPanel implements Listener {
    ...
    public void addIslandFormOverlay(Player player) {
        ...
        plugin.getFastCache().getRelations(player.getName(), relations -> {
            ...
            plugin.getFastCache().getIslandsFrom(player.getName(), result -> {
                ...
                FormWindowCustom panelIsland = new FormWindowCustom("§l§cＭＲ§fＦＡＣＥ§l§r");
                ...
                panelIsland.addElement(new ElementToggle("Khoá Lại Đảo, Không Cho Di Chuyển", false));
                panelIsland.addElement(new ElementToggle("Tự động dịch chuyển tới", true));
    ...
    public void addHomeFormOverlay(Player p, List<IslandData> listHome) {
        FormWindowSimple islandHome = new FormWindowSimple("§l§cDanh Sách Đảo§r", getLocale(p).panelHomeHeader.replace("[function]", "§aDịch chuyển đến chỗ người chơi"));
    ...
    private void addDeleteFormOverlay(Player p, IslandData pd) {
        if (pd == null) {
            plugin.getFastCache().getIslandsFrom(p.getName(), listHome -> {
                ...
                FormWindowSimple islandHome = new FormWindowSimple("Chọn Đảo Của Bạn", getLocale(p).panelHomeHeader.replace("[function]", "§a§lReset đảo của bạn?"));
        ...
        FormWindowModal confirm = new FormWindowModal("§l§cXoá Đảo Của Bạn", getLocale(p).deleteIslandSure, "§c§lXóa Đảo", "Hủy");
    ...
    private void addProtectionOverlay(Player p, IslandData pd) {
        ...
        if (pd == null) {
            plugin.getFastCache().getIslandsFrom(p.getName(), listHome -> {
                ...
                FormWindowSimple islandHome = new FormWindowSimple("Chọn đảo của bạn", getLocale(p).panelHomeHeader.replace("[function]", "§aĐặt cài đặt đảo của bạn."));
                for (IslandData pda : listHome) {
                    islandHome.addButton(new ElementButton(pda.getIslandName()));
                }
        ...
        FormWindowCustom settingForm = new FormWindowCustom("" + pd.getIslandName() + " Cài Đặt");
    ...
    private void addSettingFormOverlay(Player p, IslandData pd) {
        ...
        if (pd == null) {
            plugin.getFastCache().getIslandsFrom(p.getName(), listHome -> {
                ...
                FormWindowSimple islandHome = new FormWindowSimple("Chọn Đảo Của Bạn", getLocale(p).panelHomeHeader.replace("[function]", "§ađặt đảo của bạn."));
                for (IslandData pda : listHome) {
                    islandHome.addButton(new ElementButton(pda.getIslandName()));
                }
        ...
        settingForm.addElement(new ElementInput("Tên Đảo", "", pd.getIslandName())); // islandMaxNameLong
        settingForm.addElement(new ElementToggle("Khoá Đảo", pd.isLocked()));
        mapIslandId.put(p, pd);
